import { LayoutDashboard, LogOut, PieChart, Wallet, Briefcase, Users, Menu, X } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden">
      ...
      {/* Mobile Overlay */}
      {isMobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 bg-black/50 z-30" onClick={closeMobileMenu} />
      )}

      {/* Sidebar */}
      <aside
        className={cn(
          'w-64 border-r border-slate-800 flex flex-col bg-slate-900/50 transition-transform duration-300 ease-in-out',
      ...
      >
        <div className="p-6">
          <div className="flex items-center gap-2 mb-8">
            ...
            <span className="text-xl font-bold tracking-tight text-white">Altfolio</span>
          </div>
      ...
      <main className="flex-1 overflow-auto w-full lg:w-auto">
        <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 pt-16 lg:pt-8">{children}</div>
      </main>
    </div>
  );
};
